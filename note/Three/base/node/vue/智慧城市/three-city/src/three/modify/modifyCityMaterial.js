...
export default function modifyCityMaterial(mesh) {
  mesh.material.onBeforeCompile = shader => {
    ...
    // console.log(22, shader.fragmentShader, shader.vertexShader)
  }
}
...
export function addGradColor(shader, mesh) {
  ...
  let uHeight = max.y - min.y   // float


  shader.uniforms.uTopColor = {
    value: new T.Color('#aaaeff')
  }
...
// 添加建筑材质光波扩散特效
export function addSpread(shader, center = new T.Vector2(0, 0)) {

  // 设置扩散的中心点
  shader.uniforms.uSpreadCenter = { value: center }
  // 扩散时间
  shader.uniforms.uSpreadTime = { value: -2e3 }
  // 设置条带的宽度
...
export function addLightLine(shader) {
  ...
  // 设置条带宽度
  shader.uniforms.uLightLineWidth = { value: 2e2 }
  ...
  shader.fragmentShader = shader.fragmentShader.replace(
    ...
    `
      ...
      if(LightLineMix > 0.0) {
        gl_FragColor = mix(gl_FragColor , vec4(.8,1.0,1.0,1.0) , LightLineMix / uLightLineWidth ) ;
      }
      //#end#
    `
  )
...
export function addToTopLine(shader) {
  ...
  shader.fragmentShader = shader.fragmentShader.replace(
    ...
    `
      ...
      if(ToTopMix > 0.0) {
        gl_FragColor = mix(gl_FragColor , vec4(.8,.8,1.0,1.0) , ToTopMix / uToTopWidth) ;
      }
      //#end#
    `
  )
